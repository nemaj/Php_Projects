...
$get = $conn->query("SELECT * FROM pre_enroll");

if ($get->num_rows > 0) {
	$result = array();

	while ($row = $get->fetch_assoc()) {
		$checkId = $conn->query("SELECT * FROM enrolled WHERE pupil_id='".$row['pupil_id']."'");
		...
		if ($checkId->num_rows == 0) {
			...
			$item = array(
					'id' => $row['pupil_id'],
					'lastName' => ucfirst($infoArr['last_name']),
					'firstName' => ucfirst($infoArr['first_name']),
					'middleName' => ucfirst($infoArr['middle_name']),
